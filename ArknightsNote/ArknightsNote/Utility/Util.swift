...
enum Util {
    static func extractText(from html: String) -> String? {
        (try? Kanna.HTML(html: html, encoding: .utf8))?.text
    }

    /// Computes and extracts the range of XML elements and element contents.
    ///
    /// This function returns a dictionary that contains the range of a XML element as key
    /// and the range of a XML element content as value.
    static func extractXMLElementRange(
        from text: String,
        with pattern: String
    ...
    /// Computes and extracts XML element content from given text
    /// and the given pattern is used to recognize an XML element.
    ///
    /// This function returns a tuple
    /// where the first element is a String with all XML tags removed,
    /// and the second element contains the range of each XML element content
    /// in the first element of the returned tuple.
    static func extractXMLContent(from text: String, with pattern: String) -> (String, [Range<String.Index>]) {
        ...
        for index in 0 ..< textSegments.count where isXMLContent[index] {
            let xmlContent = textSegments[index]
